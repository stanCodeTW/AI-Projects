...
class LF2Environment():
    def __init__(self, path, ip, port, driverType, characters, difficulty, background, versusPlayer, duel, rewardList, localDriver, headless, canvasSize, debug):
        ...
        self.driver = None # obervation from web (game.html)
        self.figures = {}
        self.axs = {}
        self.ims = {}
    ...
    def quick_start(self):
        ...
        if any(ch not in config.Character for ch in self.characters): lf2raise('Character [%s] is not in the role list.')
